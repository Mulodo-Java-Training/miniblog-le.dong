		...
		if (form.comment != null && id_posts > 0) {
			...
			if (rs > 0) {
				...
				if (result > 0) {
					...
					json.meta.code(4006);
					return Response.status(Status.STATUS_4006).entity(json)
							.build();
				}
			}
		}
				...
				if (c != null) {
					if (c.getPosts().getId() == id_posts) {
						...
						if (a.getId() == c.getAccount().getId()) {
							...
							json.meta.code(4009);
							return Response.status(Status.STATUS_4009)
									.entity(json).build();
						}
					} else {
						json.meta.code(4008);
					...
					json.meta.code(4004);
					return Response.status(Status.STATUS_4004).entity(json)
							.build();
				}
			}
		}
